...
// Explosive weapons (grenades, launchers, mines) list negligible impact damage on
// the WEAP form; the Pip-Boy display uses the linked explosion's damage instead.
static float GetExplosionDamageFromProjectile(BGSProjectile *proj) {
  if (!proj || !proj->explosionForm)
    return 0.0f;
  return proj->explosionForm->damage;
}

static float GetExplosionDamageFromAmmoForm(TESForm *ammoForm) {
  if (!ammoForm || ammoForm->typeID != kFormType_TESAmmo)
    return 0.0f;
  return GetExplosionDamageFromProjectile(((TESAmmo *)ammoForm)->projectile);
}

static float GetExplosiveWeaponBaseDamage(TESObjectWEAP *weap,
                                          PlayerCharacter *player) {
  if (!weap)
    return 0.0f;

  float dmg = GetExplosionDamageFromProjectile(weap->projectile);
  if (dmg > 0.0f)
    return dmg;

  TESForm *ammoForm = weap->ammo.ammo;
  if (!ammoForm)
    return 0.0f;

  // When this launcher is equipped, use the ammo type actually loaded in the
  // chamber so displayed DAM matches the in-game Pip-Boy for that round.
  if (player) {
    TESObjectWEAP *eq = player->GetEquippedWeapon();
    if (eq && eq->refID == weap->refID && player->baseProcess) {
      BaseProcess::AmmoInfo *ammoInfo = player->baseProcess->GetAmmoInfo();
      if (ammoInfo && ammoInfo->ammo) {
        dmg = GetExplosionDamageFromAmmoForm(ammoInfo->ammo);
        if (dmg > 0.0f)
          return dmg;
      }
    }
  ...
  if (ammoForm->typeID == kFormType_TESAmmo)
    return GetExplosionDamageFromAmmoForm(ammoForm);

  if (ammoForm->typeID == kFormType_BGSListForm) {
    BGSListForm *list = (BGSListForm *)ammoForm;
    for (UInt32 i = 0; i < list->Count(); i++) {
      dmg = GetExplosionDamageFromAmmoForm(list->GetNthForm(i));
      if (dmg > 0.0f)
        return dmg;
    }
  }
...
static int ComputeWeaponDisplayDamage(PlayerCharacter *player,
  ...
  // FNV eWeaponType values (from GECK wiki / GetWeaponType):
  ...
  if (player && (wtype == TESObjectWEAP::kWeapType_OneHandMelee ||
                 wtype == TESObjectWEAP::kWeapType_TwoHandMelee ||
                 wtype == TESObjectWEAP::kWeapType_OneHandThrown)) {
    float bonus = player->avOwner.Fn_03(kAV_MeleeDamage);
  ...
  float dam = core;
  if (player && wtype == 0) {
    float unarmed = player->avOwner.Fn_03(kAV_Unarmed);
    if (unarmed < 0.0f) unarmed = 0.0f;
...
std::string BuildPlayerSnapshot() {
  ...
  json.endArray();
    ...
    for (const auto &pair : agg) {
      ...
      // Weapons carry a condition/skill-adjusted display damage so the Pip-Boy
      // can show the same dynamic DAM the game does. Omitted (Pip-Boy keeps its
      // static DAT value) for non-weapons or weapons with no base damage.
      if (item.baseForm &&
          item.baseForm->typeID == kFormType_TESObjectWEAP) {
...
// equip/unequip. EquipItem/UnequipItem reset the scrollbar's live scroll
// value to 0 as a side effect of the underlying inventory change, and that
// happens before Refresh() ever runs - so the value must be captured
// before calling EquipItem/UnequipItem and reapplied right before Refresh().
...
// 0x95EA30's tail, exactly: resolve the item's
...
static void ExecutePipBoyCommand(const std::string &line) {
  ...
  // Capture the ITEMS list scroll position before
  // EquipItem/UnequipItem runs below -
  // it resets the scrollbar's trait to 0 as a side effect,
  // so reading it any later always sees 0.
